...
- `make share` — **открыть доступ к игре из интернета через Ngrok**

## 🌐 Как играть с друзьями по интернету

По умолчанию игра запускается только локально. Чтобы ваш друг мог к вам присоединиться, вам нужно "пробросить туннель" через интернет.

1. Установите [Ngrok](https://ngrok.com/download) и авторизуйтесь (выполните `ngrok config add-authtoken <ВАШ_ТОКЕН>`).
2. Убедитесь, что игра запущена (`make up`).
3. В новом терминале выполните:
   ```bash
   make share
   ```
4. Ngrok выдаст вам зеленую ссылку вида `https://1234-abcd.ngrok-free.app`.
5. Перейдите по ней, создайте комнату и отправьте финальную ссылку другу! Запросы (в том числе WebSocket) будут автоматически маршрутизироваться.
